...
for i in range(len(line_2)):
	try:
		if (line_2[i - 2].islower() and line_2[i - 1].islower() and line_2[i].isupper()) or len(line) > 0:
			line += line_2[i]
...
with open('newfile.txt', 'r', encoding='utf-8') as f:
    ...
    seq = re.findall(pattern, number)
    long_seq = []
    [long_seq.insert(0, x) for x in seq if len(x) > len(long_seq)]
print(f"Самая длинная последовательность одинаковых циф в файле = {long_seq[0]}")
