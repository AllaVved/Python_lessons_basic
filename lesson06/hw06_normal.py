...
# 2. Получить список всех учеников в указанном классе
	def get_list_students(class_rooms):
		for st in students:
			if st.class_room == class_rooms:
				print(st.get_surname_initials()) 

	
class Student(Person):
	def __init__(self, surname, name, patronymic, birth_date, class_room, parents):
		Person.__init__(self, surname, name, patronymic, birth_date)
		self.class_room = class_room
...
#Не работает ошибка AttributeError: 'NoneType' object has no attribute 'subject'
#Если делать return у teacher_in_class_rooms то ошибка AttributeError: 'str' object has no attribute 'subject'
#Teacher.sub(Teacher.teacher_in_class_rooms(students[0].class_room))
